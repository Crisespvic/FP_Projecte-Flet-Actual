...
def processar_pregunta(user_text):
    ...
    if resposta.get("success"):
        ...
        tipus_resposta = resposta.get("type")
        
        # El backend ens envia un missatge de text (perquè falten filtres o no hi ha dades)
        if missatge_backend:
            return missatge_backend, tipus_resposta
        
        # No hi ha dades i tampoc hi ha missatge
        if not dades:
            return "No he trobat resultats per a aquesta consulta. Prova a especificar una altra zona."

        # Hi ha dades, comprovem el format
        primer_item = dades[0]
        if "centre" in primer_item or "grau" in primer_item: # "centre" és la clau que usa SQL
            return [Targeta.from_json(item) for item in dades], tipus_resposta
